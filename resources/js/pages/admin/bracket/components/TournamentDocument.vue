...
<template>
    ...
    -->
    <Card class="print:border-none print:shadow-none overflow-hidden shadow-sm bg-card border-border">
        <CardHeader class="border-b bg-muted/50 flex flex-row items-center justify-between space-y-0 py-4">
            <div class="space-y-1">
            ...
            <Button variant="outline" size="sm" class="print:hidden border-border" @click="exportPdf">
        ...
        <CardContent class="p-0">
            ...
            <div class="max-h-150 overflow-auto print:max-h-none print:overflow-visible">
                <Table>
                    <TableHeader class="bg-muted/50 sticky top-0 z-10 backdrop-blur-sm print:static">
                        <TableRow class="hover:bg-transparent border-b border-border">
                            <TableHead class="w-16 font-semibold text-muted-foreground">Order</TableHead>
                            ...
                            <TableHead class="text-center font-semibold text-muted-foreground">Status</TableHead>
                        </TableRow>
                    </TableHeader>
                    <TableBody>
                        <TableRow 
                            ...
                            :key="`doc-${m.id}`"
                            class="hover:bg-muted/50 transition-colors border-b border-border group"
                        >
                            ...
                            <TableCell>
                                <span :class="{'font-bold text-primary': m.winner_id === m.player_one_id}">
                                    {{ m.player_one || 'BYE' }}
                                </span>
                            </TableCell>
                            <TableCell>
                                <span :class="{'font-bold text-primary': m.winner_id === m.player_two_id}">
                                    {{ m.player_two || 'BYE' }}
                                </span>
                            </TableCell>
                            <TableCell class="text-center">
