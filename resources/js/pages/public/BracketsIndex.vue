...
<div class="min-h-screen bg-background text-foreground font-sans selection:bg-accent selection:text-accent-foreground">
    ...
    <main class="max-w-7xl mx-auto px-4 py-16 relative">
            ...
            <div class="flex flex-col items-center text-center gap-6 border-b border-border pb-12">
                <div class="max-w-4xl mx-auto flex flex-col items-center">
                    <div class="flex items-center gap-3 mb-4 justify-center">
                        ...
                        <span class="text-accent font-black uppercase tracking-[0.3em] text-[10px]">Official Brackets</span>
                        <div class="h-px w-8 bg-accent"></div>
                    </div>
                    <h1 class="text-5xl md:text-7xl font-serif font-bold text-foreground mb-6 leading-tight tracking-tight uppercase">
                        Brackets <span class="text-accent italic">Board</span>
                    </h1>
        ...
        <!-- Toolbar -->
        <div class="flex flex-col md:flex-row items-center justify-between gap-6 mb-12 bg-card/50 p-2 rounded-2xl border border-border backdrop-blur-sm relative z-10">
            <!-- Tabs -->
            <div class="flex items-center bg-muted/50 rounded-xl p-1 gap-1 w-full md:w-auto overflow-x-auto">
                <button v-for="tab in tabs" :key="tab"
                    ...
                    :class="[
                        ...
                        activeTab === tab ? 'bg-accent text-accent-foreground shadow-lg shadow-accent/20' : 'text-muted-foreground hover:text-foreground hover:bg-background/50'
                    ]">
                    {{ tab }}
                </button>
            </div>
            ...
            <div class="flex items-center gap-3 w-full md:w-auto">
                <div class="relative group w-full md:w-64">
                    ...
                    <input 
                        ...
                        class="w-full bg-muted/50 border border-border rounded-xl py-2.5 pl-10 pr-4 text-sm text-foreground placeholder-muted-foreground focus:outline-none focus:border-accent/50 focus:ring-1 focus:ring-accent/50 transition-all" 
                    />
                </div>
            </div>
        </div>
        ...
        <!-- Tournaments Grid -->
        <div v-if="filteredTournaments.length === 0" class="py-24 text-center border border-dashed border-border rounded-5xl">
            <Trophy class="w-16 h-16 text-muted-foreground mx-auto mb-6 opacity-20" />
            <p class="text-muted-foreground italic text-xl">No tournament brackets found.</p>
        </div>
        ...
        <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            <Link v-for="tournament in filteredTournaments" 
                  ...
                  class="group relative bg-card/40 rounded-4xl border border-border hover:border-accent/30 transition-all duration-500 overflow-hidden flex flex-col h-full shadow-2xl hover:shadow-accent/5"
            >
                <!-- Card Header/Image Area -->
                <div class="h-48 bg-muted/50 relative overflow-hidden">
                ...
                <div class="p-8 flex-1 flex flex-col">
                    <div class="flex items-center gap-2 text-accent/60 font-bold text-[10px] uppercase tracking-[0.2em] mb-4">
                    ...
                    <div class="grid grid-cols-2 gap-4 mt-auto">
                        <div class="bg-muted/30 rounded-2xl p-4 border border-border/50">
                            <div class="text-[10px] font-black text-muted-foreground uppercase tracking-widest mb-1">Categories</div>
                            <div class="text-xl font-bold text-foreground">{{ tournament.brackets_count }}</div>
                        </div>
                        <div class="bg-muted/30 rounded-2xl p-4 border border-border/50 flex items-end justify-between group/btn">
                            <div class="text-[10px] font-black text-accent uppercase tracking-widest mb-1">View</div>
                            <ChevronRight class="w-5 h-5 text-accent transform group-hover/btn:translate-x-1 transition-transform" />
                        </div>
                    </div>
                </div>

                <!-- Hover Decor -->
                <div class="absolute bottom-0 left-0 w-0 h-1 bg-accent transition-all duration-500 group-hover:w-full"></div>
            </Link>
        </div>
