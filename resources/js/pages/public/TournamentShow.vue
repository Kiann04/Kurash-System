...
<div class="min-h-screen bg-[#050a14] text-white font-sans selection:bg-yellow-500 selection:text-black">
    ...
    <main class="max-w-7xl mx-auto px-4 py-12 relative">
        ...
        <div class="mb-12 flex justify-center">
            <div class="bg-white/5 p-1.5 rounded-2xl inline-flex relative">
                <button 
                ...
                <div v-else v-for="group in maleFightCardGroups" :key="`male-${group.name}`" class="space-y-8">
                    ...
                    <div class="space-y-4">
                        <div v-for="match in group.matches" :key="match.id" 
                             class="group bg-[#0f172a] rounded-2xl border border-slate-800 hover:border-blue-500/30 transition-all duration-300 overflow-hidden hover:shadow-lg hover:shadow-blue-900/10">
                            <div class="flex flex-col md:flex-row items-stretch">
                                <!-- Match Info Sidebar -->
                                <div class="w-full md:w-32 bg-black/20 border-b md:border-b-0 md:border-r border-slate-800 p-4 flex flex-row md:flex-col items-center md:items-start justify-between md:justify-center gap-2">
                                     <div class="flex items-center gap-2 md:flex-col md:items-start">
                                        <span class="text-[10px] font-black uppercase tracking-widest text-slate-500">Match</span>
                                        <span class="text-xl font-serif font-bold text-white">{{ match.match_number }}</span>
                                     </div>
                                     <div class="flex items-center gap-2 md:flex-col md:items-start">
                                        <div v-if="match.status === 'ongoing'" class="px-2 py-1 rounded bg-orange-500/10 border border-orange-500/20 text-[10px] font-bold text-orange-500 uppercase tracking-wider flex items-center gap-1.5">
                                            <span class="w-1.5 h-1.5 rounded-full bg-orange-500 animate-pulse"></span>
                                            Live
                                        </div>
                                        <div v-else :class="['px-2 py-1 rounded text-[10px] font-bold uppercase tracking-wider', match.status === 'completed' ? 'bg-slate-800 text-slate-400' : 'bg-blue-500/10 text-blue-400']">
                                            {{ match.status }}
                                        </div>
                                     </div>
                                </div>
                                ...
                                <div class="flex-1 p-6 flex flex-col md:flex-row items-center gap-8 md:gap-12">
                                    ...
                                    <div class="flex-1 flex items-center justify-end gap-4 w-full">
                                        <div class="text-right">
                                            <h4 class="text-lg md:text-xl font-serif font-bold text-white group-hover:text-blue-400 transition-colors leading-tight">{{ match.player_one || 'TBD' }}</h4>
                                            <div class="text-[10px] font-bold uppercase tracking-widest text-slate-500 mt-1">{{ match.player_one_club || 'Independent' }}</div>
                                        </div>
                                        <div class="relative shrink-0">
                                             <div class="w-12 h-12 md:w-16 md:h-16 rounded-full overflow-hidden border-2 border-slate-700 group-hover:border-blue-500 transition-colors bg-slate-800">
                                             ...
                                             <div v-if="match.winner_id === match.player_one_id" class="absolute -bottom-1 -right-1 w-5 h-5 md:w-6 md:h-6 rounded-full bg-yellow-500 flex items-center justify-center text-black border-2 border-[#0f172a]">
                                    ...
                                    <div class="shrink-0 flex flex-col items-center justify-center gap-1">
                                         <span class="text-2xl font-serif font-black italic text-slate-700/50">VS</span>
                                         <span class="text-[10px] font-bold uppercase tracking-widest text-slate-600">{{ match.category_name }}</span>
                                    </div>
                                    ...
                                    <div class="flex-1 flex flex-row-reverse md:flex-row items-center justify-end md:justify-start gap-4 w-full">
                                        <div class="relative shrink-0">
                                             ...
                                             <div v-if="match.winner_id === match.player_two_id" class="absolute -bottom-1 -left-1 w-5 h-5 md:w-6 md:h-6 rounded-full bg-yellow-500 flex items-center justify-center text-black border-2 border-[#0f172a]">
                                                <Trophy class="w-3 h-3" />
                                             </div>
                                        </div>
                                        <div class="text-left">
                                            <h4 class="text-lg md:text-xl font-serif font-bold text-white group-hover:text-blue-400 transition-colors leading-tight">{{ match.player_two || 'TBD' }}</h4>
                                            <div class="text-[10px] font-bold uppercase tracking-widest text-slate-500 mt-1">{{ match.player_two_club || 'Independent' }}</div>
                                        </div>
                                    </div>
                                </div>
        ...
        <div v-if="currentView === 'female'" class="space-y-24">
            ...
            <div class="space-y-16">
                ...
                <div v-else v-for="group in femaleFightCardGroups" :key="`female-${group.name}`" class="space-y-8">
                    ...
                    <div class="space-y-4">
                        <div v-for="match in group.matches" :key="match.id" 
                             class="group bg-[#0f172a] rounded-2xl border border-slate-800 hover:border-emerald-500/30 transition-all duration-300 overflow-hidden hover:shadow-lg hover:shadow-emerald-900/10">
                            <div class="flex flex-col md:flex-row items-stretch">
                                <!-- Match Info Sidebar -->
                                <div class="w-full md:w-32 bg-black/20 border-b md:border-b-0 md:border-r border-slate-800 p-4 flex flex-row md:flex-col items-center md:items-start justify-between md:justify-center gap-2">
                                     <div class="flex items-center gap-2 md:flex-col md:items-start">
                                     ...
                                     <div class="flex items-center gap-2 md:flex-col md:items-start">
                                        ...
                                        <div v-else :class="['px-2 py-1 rounded text-[10px] font-bold uppercase tracking-wider', match.status === 'completed' ? 'bg-slate-800 text-slate-400' : 'bg-emerald-500/10 text-emerald-400']">
                                            {{ match.status }}
                                        </div>
                                     </div>
                                </div>
                                ...
                                <div class="flex-1 p-6 flex flex-col md:flex-row items-center gap-8 md:gap-12">
                                    ...
                                    <div class="flex-1 flex items-center justify-end gap-4 w-full">
                                        <div class="text-right">
                                            <h4 class="text-lg md:text-xl font-serif font-bold text-white group-hover:text-emerald-400 transition-colors leading-tight">{{ match.player_one || 'TBD' }}</h4>
                                            <div class="text-[10px] font-bold uppercase tracking-widest text-slate-500 mt-1">{{ match.player_one_club || 'Independent' }}</div>
                                        </div>
                                        <div class="relative shrink-0">
                                             <div class="w-12 h-12 md:w-16 md:h-16 rounded-full overflow-hidden border-2 border-slate-700 group-hover:border-emerald-500 transition-colors bg-slate-800">
                                                <img :src="match.player_one_image ? `/storage/${match.player_one_image}` : '/images/default-profile.svg'" class="w-full h-full object-cover" />
                                             </div>
                                             <div v-if="match.winner_id === match.player_one_id" class="absolute -bottom-1 -right-1 w-5 h-5 md:w-6 md:h-6 rounded-full bg-yellow-500 flex items-center justify-center text-black border-2 border-[#0f172a]">
                                                <Trophy class="w-3 h-3" />
                                    ...
                                    <div class="flex-1 flex flex-row-reverse md:flex-row items-center justify-end md:justify-start gap-4 w-full">
                                        ...
                                        <div class="text-left">
                                            <h4 class="text-lg md:text-xl font-serif font-bold text-white group-hover:text-emerald-400 transition-colors leading-tight">{{ match.player_two || 'TBD' }}</h4>
                                            <div class="text-[10px] font-bold uppercase tracking-widest text-slate-500 mt-1">{{ match.player_two_club || 'Independent' }}</div>
                                        </div>
                                    </div>
                                </div>
